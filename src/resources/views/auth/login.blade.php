...
<div class="content">
    ...
    <div class="create-form__items">
        <form class="create-form" action="{{ route('login') }}" method="post">
            @csrf
            <div class="create-form__items-name">
                <h3 class="create-form__items-ttl">メールアドレス</h3>
                <input class="create-form__items-input" type="text" placeholder="例:test@example.com" name="email" value="{{ old('email') }}">
            </div>
            @if($errors->has('email'))
            <tr>
                <td>
